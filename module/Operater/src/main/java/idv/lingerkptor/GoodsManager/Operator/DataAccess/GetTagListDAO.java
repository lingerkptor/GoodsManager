...
public class GetTagListDAO implements PreparedStatementCreator {
	private Properties prop = new Properties();
	private int GID = 0;

	public GetTagListDAO(int GID) {
		this.GID = GID;
	}
	...
	public PreparedStatement createPreparedStatement(Connection conn) throws SQLException {
		...
		if (GID > 0) {
			SQL = prop.getProperty("getTagsbyGID");
			stat = conn.prepareStatement(SQL);
			stat.setInt(1, GID);
		} else {
			SQL = prop.getProperty("listAllTag");
			stat = conn.prepareStatement(SQL);
		}
		stat.addBatch();
		stat.execute();
